...
const curlIncludeDirectory = join(root, "vendor", "include", "curl");
const headerNames = (await readdir(curlIncludeDirectory)).filter((name) => name.endsWith(".h"));
const curlHeader = (
  await Promise.all(
    headerNames.map(async (name) => readFile(join(curlIncludeDirectory, name), "utf8")),
  )
).join("\n");
const shimHeader = await readFile(join(root, "crates", "native", "c", "shim.h"), "utf8");
...
for (const match of bindings.matchAll(/pub const (CURLOPT_[A-Z0-9_]+): CURLoption = (\d+);/g)) {
...
for (const [name, committed] of committedOptions) {
  ...
  if (current === undefined) mismatches.push(`${name} is absent from curl.h`);
    ...
    mismatches.push(`${name}: bindings=${committed}, curl.h=${current}`);
}
